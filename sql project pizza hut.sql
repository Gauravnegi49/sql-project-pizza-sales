...
SELECT 
    HOUR(order_time) AS hour, COUNT(order_id) AS orderno
FROM
    orders
...
FROM
    ...
    FROM
        orders
    JOIN orders_details ON orders.order_id = orders_details.orders_id
...
join orders
on orders.order_id= orders_details.orders_id
